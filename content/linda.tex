...
Ehhez a célhoz egyszerűségét két dologból meríti.
...
Ezek között találhatóak több esetben a HPC környezetben klasszikusan előforduló nyelveken kívül, elsősorban nagyvállalati kontextusban használt nyelvek is; ilyenek többek közt a Java \cite{sun_microsystems_javaspaces_2011, lehman_hitting_2001}, de még szkript nyelvekhez, mint például a Ruby \cite[Part 3 -- Rinda]{seki_druby_nodate} és a Python \cite{andrew_wilkinson_pylinda_2007}, is készültek implementációk.

Mivel az általam a következőkben adott megvalósítás C++ nyelven készült, ezért fontos kitérni az ezen a nyelven már megvalósított régebbi rendszerekre.
Ezek a Boreas \cite{naval_aeriboreas_2024} és CppLinda \cite{thomas_arkadius_sluga_modern_2007}.
Ezekkel az implementációkkal akadnak azonban problémák.
A Boreas pusztán az adattárolást valósítja meg egy elosztott \ts{} formájában, a nyelvbe épített párhuzamosítást (\ld \lcmd{eval}, \ref{def:linda-cmd}.~szekció) nem.
Ennek megfelelően, nem tekinteném egy teljes értékű Linda megvalósításnak.
A CppLinda biztosítja ugyan a teljes Linda eszköztárat, mára a kódbázisa öregnek számít.
Eredetileg csak Linux alapú operációs rendszereken működött a használt operációs rendszer szintű függvényhívások miatt, amivel a kommunikációt és folyamatindítást valósítja meg.
Modern rendszereken azonban, se asztali gépen, se szuperszámítógépen már sajnos nem fordítható még Linux alatt sem, a kódbázisának módosítása nélkül.
...
Ezek mindegyike egy--egy egyszerű részfeladatot old meg a párhuzamos működés megvalósítása érdekében: az összes processzel megosztott \un\nes{} tér (\textit{\ac{TS}}, innentől, az eredeti publikációt követve csak \acs{TS}, \cite{ahuja_linda_1986}) állapotának olvasását vagy módosítását teszik lehetővé a programozó számára.
Ebben látszik egy osztott memóriás világbeli szemlélet: minden program és programrészlet, ugyanazt az \ts{}-t látja, és az ezen végzett módosításokon keresztül tudnak adatot átadni egymásnak.
Az osztott memória azonban sok extra munkát és odafigyelést igényel a programozó részéről.
Biztosítani kell, hogy az adott memóriaterületek írása/olvasása megfelelően legyen szinkronizálva a különböző számítási folyamatok között.
A gyakran előforduló problémák közül az egyik leggyakoribbak a versenyhelyzetek ({\selectlanguage{english}race condition}) és a pattok ({\selectlanguage{english}deadlock}), amelyek mindegyike a nem megfelelő zárolásból adódik a program \un{} kritikus szekcióiban.
...
Az előző bekezdésben a \ts, mint elsődleges eleme a Linda modelljének került bevezetésre. Ennek a pontos definiáláshoz azonban először szükség van pár alap-építőelemre a Linda eszköztárban:
...
\begin{defn}[Linda típus]\label{def:linda-type}\index{Linda típus}
    ...
    \begin{enumerate}
        \item Minden $\mathbb{T}$ típusú érték egyértelműen összehasonlítható egyenlőség operátorral.
...
\begin{remark}
    \Aref{def:linda-type}.~definícióból látható, hogy a megadott halmaz mérete és konkrét elemei csupán a megvalósítástól függenek: a modell csak azokat a tulajdonságokat írja elő, amivel 1) a keresést tudja megvalósítani és 2) más folyamatoknak tudja átadni az értéket.
\end{remark}

\begin{defn}[Linda érték]\label{def:linda-value}\index{Linda érték}
    Olyan érték, amely típusa egy Linda típus.
\end{defn}
...
Ezek azok a szekció elején említett egyszerű műveletek, amelyeket a Linda beilleszt egy választott gazdanyelvbe, hogy azt párhuzamos programozási nyelvvé változtassa. Ezek definíciója látható \aref{def:linda-cmd}.~definícióban.
...
\begin{defn}[Linda műveletek]\label{def:linda-cmd}\index{Linda művelet}
    ...
    \begin{description}
        ...
        Ha nem talál ilyen \nes{}t \ats{}--ben, akkor addig várakozik, ameddig egy megfelelő érték meg nem jelenik a \ts{}--ben.
        ...
        Ha nem talál ilyet, akkor hamis értékkel tér vissza, és nem módosítja a \ts--t.
        ...
        Ha nem talál ilyet, akkor hamis értékkel tér vissza.
...
\begin{figure}[bt!]
	\centering
	\resizebox{9.5cm}{!}{\includegraphics{figures/linda-in.pdf}}
	\caption{A Linda \lcmd{in} és \lcmd{inp} művelet}
	\label{fig:linda-in}
\end{figure}

Az műveletek megadásában található $tuple...$, egyes Linda értékekkel megadott Linda \nes{}, hozzávéve még \aref{subsec:qm-op}.~szekcióban ismertetett lehetőséget.
Az \lcmd{eval}-ban látható $tuple^*...$ paraméter értelmezésére a következőkben fog szó esni \aref{subsec:eval-fn}.~szekcióban.

Míg az \lcmd{out} működése egyszerű és egyértelmű, az \lcmd{in(p)} és \lcmd{rd(p)} függvények megértését segítik \aref{fig:linda-in}.\ és \aref{fig:linda-rd}.~ábrák.
...
\begin{figure}[thb]
    \centerline{\resizebox{9.5cm}{!}{\includegraphics{figures/linda-rd.pdf}}}
...
\begin{figure}[htb]
    \centerline{\resizebox{6cm}{!}{\includegraphics{figures/query.pdf}}}
...
\begin{figure}[htb]
    \centerline{\resizebox{7.5cm}{!}{\includegraphics{figures/linda-eval.pdf}}}
...
Mivel a feladat fontos része, hogy modern és hordozható eszközök kerüljenek felhasználásra, így a megvalósítás C++-al, ezen belül is a 2020-ban kiadott szabványnak megfelelő verzióval történt (ISO/IEC 14882:2020).
Ezen Linda megvalósítás egy statikus könyvtárrá fordul, amely magában hordoz minden szükséges extra függőséget, használatához pusztán ezt a könyvtárat kell linkelni a programhoz.
A fordítás Clang ($17.0.3$) és MSVC ($19.40.33807$) fordítókkal működik és ezen fordítók adott verzióikkal lett tesztelve.

A többszálúság megvalósítására a C++ által biztosított szálkezelési primitívek kerültek felhasználásra, így ehhez nincs szükség külső könyvtárra.
A szálak \texttt{std::thread} (és \texttt{std::jthread}) objektumokként vannak kezelve, amelyek a szabványos könyvtárban definiált szinkronizációs eszközöket használják fel az egymás közötti koordináció megvalósítására.
Ezek mellett az \texttt{std::atomic<T>} sablon is felhasználásra került pár zárolás--mentes algoritmus megvalósítására, pl.\ egy folyamat szintű monoton számláló implementálására.

Fontos kitérni rá, hogy miért nem döntöttem az OpenMP használata mellett.
Ez egy elterjedt könyvtár a közös memóriával rendelkező párhuzamosításra.
Használata nem növelte volna annyiban a hordozhatóságot, amit eleve a C++ szabványhoz kötöttem, mint amennyivel nehezítette volna a megvalósítást és a kód olvashatóságát.
Így a szál--párhuzamosított részei a programnak nagy részben  \aref{subsec:ranks}.~szekcióban tárgyalt thread--pool megvalósítás részeként üzemelnek, ami mellett egy--egy segéd szál végez még érdemi munkát.
...
A  párhuzamosítható ciklusokat, a C++ által biztosított \texttt{std::execution::par\_unseq} direktívával ellátott szabványos algoritmusokkal valósítottam meg.
...
Manuálisan megvalósított folyamatindításokkal ez jelentősen nehezebben, vagy egyáltalán nem valósítható meg, és a különböző optimalizációkat is elveszítjük, amelyet a konkrét hardverhez tartozó MPI környezet biztosítana.
...
Nem elhanyagolható előnye pedig, hogy egy ORDBMS ({\selectlanguage{english}Object-Relational Database Management System}), azaz objektum-relációs adatbázis kezelő rendszer: lehetővé teszi, hogy saját összetett típusokat lehessen definiálni, amelyeknek a tárolására alkalmas.
Ezek lehetnek egyszerű struktúrák, de teljesen saját típusokat is lehetséges megvalósítani saját konverziós függvények megadásával.
Sőt a nagyon különleges viselkedésű típusokra még az optimalizáló számára is lehetséges segédfüggvényeket definiálni a típus hatékony használatáról.
Arról, hogy ezeket a lehetőségeket hogy használja ki a megvalósítás, több szó esik
\aref{subsec:the-lts}.~szakaszban.

\subsection{Eltérések a modelltől}\label{subsec:diff-to-ideal}

Az adott megvalósításban próbáltam a legközelebb maradni a bemutatott Linda elméleti modellhez.
Két helyen azonban kompromisszumos megoldást kellett alkalmazni, a használt nyelv rugalmatlansága miatt.

Az első a ?--operátor, ahogy \aref{subsec:qm-op}.~szekcióban bemutattam, sajnos nem megvalósítható C++ nyelven.
...
Ugyanakkor, ha valahogyan még meg is lehetne oldani valamiféle rafinált előfeldolgozási lépésekkel, hogy kettőspontok a kód megfelelő részeire bekerüljenek, akkor is szembesülnénk azzal a problémával, hogy ezt az operátort nem lehet felüldefiniálni, amivel ez az egész irány zsákutcává változik.

Ezt megoldandó, egy saját függvény bevezetésére mellett döntöttem:
az \texttt{ldb::ref}, egy mutatót vesz át arra a változóra, amelyiket a lekérdezésben szerepeltetni szeretnénk, és készít belőle egy olyan belső típust, amelyik alapján a lekérdezés végrehajtó tudja, hogy egyenlőség illesztéskor, ebbe a mutatóba bele kell írni az illeszkedő \nes{} megfelelő pozíciójú értékét.
A név a szabványos könyvtárban található \texttt{std::ref} függvényre utal.
...
Ennek megfelelően meg kell szakítani a függvényhívást az \lcmd{eval} paraméterlistájában, és átalakítani egy másik folyamatban lefutó, de azonos jelentésű függvényhívássá.

Ennek a helyzetnek az eredetihez legközelebbi megoldásához a C++ makróihoz kell fordulni.
Ennek oka, hogy a sablonok szintjén a függvényhívás, mint művelet,
nem állítható vissza egy függvénnyé, amit meghívnak, és egy híváshoz tartozó paraméterhalmazzá.
Sajnos teljes kompatibilitás még a makrók felhasználásával sem megoldható, mert az előzőekben említett függvény és paraméterlista kettest nem lehet előállítani egy tényleges függvényhívás szintaktikájából.
...
Ezekből az adatokból megkonstruálható egy globális {\selectlanguage{english}\ac{LUT}}, amelyik az egyes függvények neveihez rendeli a függvénymutatókat.
...
\subsection{Átfogó architektúra}\label{subsec:lrt-arch}

A megvalósítás alapvetően egy MPI alapú párhuzamos rendszer.
Ez lehetővé teszi, több folyamat induljon el egy számítás végrehajtására, majd hogy ezek címzett üzeneteket küldjenek egymásnak.
Minden MPI processz, saját magába foglal egy végrehajtó egységet, amely a különböző Linda \lcmd{eval} által indított folyamatokat futtatja.
Emellett található egy külön folyamatban elhelyezkedő adatbázis rendszer, amelyik a \ts{} feladatát látja el.
Erről ad \aref{fig:linda-arch}.~ábra áttekintést, amelyen 3 MPI processz közötti lehetséges kommunikációt szemléltet egy \ac{UML} kommunikációs diagramot felhasználva.
A zölddel jelölt lifeline--ok az MPI kommunikátor részei, míg a kék szín a PostgreSQL folyamatban elhelyezkedő elemei a rendszernek.
Látható, hogy az \lcmd{eval} üzenetek a különböző processzek között direkt kerülnek eloszlásra, míg a primitív adat írás/olvasás műveletek egyenesen a postgres folyamat felé kerülnek irányításra.

\begin{figure}[htb]
    \centerline{\resizebox{10cm}{!}{\includegraphics{figures/linda-arch.pdf}}}
    \caption{A Linda megvalósítás kommunikációs diagramja}
    \label{fig:linda-arch}
\end{figure}

\subsection{Egy MPI processz feladata}\label{subsec:ranks}

Az előzőekben már olvasható módon, egy processz egy magában teljes számítási egységként viselkedik a globális számítás egy--egy részfeladatát végrehajtva.
Ehhez az architektúrához természetesen illeszkedik az MPI processzek egyenrangúsága, és hogy alapvetően minden processz ugyanazt a kódot kezdi el futtatni.
A Linda rendszer ezt jelentősen nem módosítja, az indított kód tényleg azonos a processzek között, egy apró különbséggel.

A $0$ azonosítójú processz egy speciális feladatot hajt végre, az szolgál a párhuzamos végrehajtás soros kezdőpontjaként.
...
Hogy ezt lehetővé tegye a megvalósítás, az MPI ellentétes viselkedése ellenére, a belépési pontot csak specifikusan a $0.$ processz futtatja, és a többi pedig várakozik arra, hogy az lefusson, amivel a teljes program végét jelzi.
Megjegyzendő, hogy ez nem zárja ki egyéb \lcmd{eval} parancsok végrehajtásától, a többszálú végrehajtásnak köszönhetően ezek egyszerre történhetnek egy MPI processzen.

Ezt a speciális esetet leszámítva, minden processz azonos feladatot lát el, és azonos kódot futtat.
...
Azt, hogy melyik processznek küldi el egy \lcmd{eval}--t elindító folyamat az üzenetet azt helyileg dönti el minden folyamat, amikor erre kerül a sor.
Ezzel a lokális döntéssel megspórolható egyéb kommunikáció, ami esetleg csak metaadatokat vinne a folyamatok között.
...
\Aref{subsubsec:serial}.~szekció szerinti sorosítás mezőnkénti végrehajtása után, a Linda \nes{} egyszerűen a \mbox{PostgreSQL} szerver felé kerül elküldésre.
Itt azonban nem a teljes \nes{}t sorosítjuk, hanem mezőnként az értékeket.
Ennek a magyarázata, hogy az adatbázis kezelőnek egy SQL lekérdezést kell küldeni, amelyet az értelmez és végrehajt.
...
Az MPI környezet lehetővé teszi a fejlesztők számára, hogy saját típusokat hozzanak létre.
Ez elsődlegesen hatékonysági célt szolgál, hiszen lehetővé teszi az MPI számára, hogy nem primitív típusokat tartalmazó üzeneteket másolás nélkül elküldjön.
Az alapötlet, hogy ezt felhasználva lehetséges megadni egy olyan leírást az MPI megvalósításnak, hogy az másolás nélkül ki tudja olvasni egy memóriapufferből az elküldendő adatokat.
Bár ez egy hatékonyságot jelentősen növelő és hasznos lehetőség, sajnos a jelenlegi célokra nem használható ki.
A Linda rendszer típusok szintjén mutatott dinamikusságát már nem tudja lekövetni: dinamikus hosszúságú tömbök (Linda \nes{}), amelyek egyesével lehetnek különböző típusúak és méretűek, ráadásul ezen mezők között is lehet olyan, amelyik dinamikus hosszúságú (sztring mező), de még rekurzívan másik \nes{}t tartalmazó is (függvényhívás jelölő).
...
Ezt az információt felhasználva, hozzávéve egyes metaadatokat tartalmazó fejléceket az \nes{} és az egyes mezők szintjén, lefoglalunk ennyi memóriát, majd ismételt bejárással belemásoljuk az egyes értékeket.
...
Ezt a jelenlegi megvalósítás, az előzőekben már bemutatott módon, a PostgreSQL adatbázist felhasználva valósítja meg.

A PostgreSQL támogatja, hogy saját szerveroldali bővítményt valósítsunk meg, amelyben teljesen saját SQL--ből használható függvényeket lehet definiálni, sok különböző támogatott nyelvet felhasználva, például (alapból beépítve) PL/pgSQL, Perl, Tcl vagy Python.
...
Ezt használja ki a Linda \ts{}, és a C \ac{ABI} elvárásainak megfelelő interfészen keresztül ad egy C++ megvalósítást a szükséges függvényekre.
...
A bővítmény magában elég egyszerű, a \ts{} minimális funkcionalitását valósítja itt meg.
Összes feladata, hogy a különböző processzek által küldött \nes{} objektumokat eltárolja és lehetővé tegye ezek között a keresést.
Ehhez két típust definiál.
...
A konkrét értékek a biztosított típusok esetén láthatóak a függelék \aref{tbl:type-serial}.~táblázatában.
Az értékek konkrétan nem kerülnek tárolásra, csak a kereséses során vannak felhasználva, amikor olyan lekérdezés fut, amelyben egy adott típust adtunk csak meg, és nem egy konkrét Linda értéket.
Ez feleltethető meg az eredeti ?--operátoros kifejezésnek.
...
Ha ezt nem tenné az adatbázis és karakterenként tároljuk el egy 64--bites szám értékét, akkor $64/8 = 8$ számjegyből álló szám esetén már több adatot tárolunk a sztring formátumban, mint számként.
Ami jelentősebb probléma azonban, hiszen a \hbox{diszk/memória} relatíve olcsó, hogy az adatokat össze is kell hasonlítani.
Két szám esetén (szómérettől függően) nagyságrendileg egy 1-2 skalár utasítással megadható azoknak a relációja.
Szövegként tárolt értékek esetén viszont minden karaktert egyesével kell összehasonlítani, esetleg vektorizálással kell gyorsítani azt.
Ez minden esetben jelentősen több munka, mint egy egyszerű összehasonlító utasítás lefuttatása, nem is számolva a manuális \ac{SIMD} kód kimagaslóan ,,jó'' karbantarthatóságával.
...
Erről a formátumról lásd \aref{subsubsec:serial}.~szekciót.
...
Ami még érdekes az adatbázisból a megvalósítás szempontjából, az a keresés.
Ehhez az aritmetikai relációs műveleteket kell megvalósítani, és hozzárendelni az elvárt operátorokhoz ($<, <=, =, >=, >$).
A rendezettséget elsőként a típus alapján adjuk meg, és ha azok azonosak, akkor érték szerinti összehasonlítást végzünk.
Ezeket az operátorokat felhasználva pedig létrehozható egy olyan \un operátor osztály (operator class), amelyik egy B--fa alapú index megvalósítását teszi lehetővé az LV típus felett, gyorsítva a keresést.
...
Előbbit egyszerű megvalósítani, lényegében egy szimpla adatbázis lekérdezéssé alakítandó a keresett \nes{} úgy, hogy az adott indexű Linda érték illeszkedését nézzük az azonos indexű oszloppal.
Amire figyelni kell, hogy az egy \nes{} keresésénél a reláció $n + 1$. mezejét ellenőrizni kell, hogy \texttt{NULL}, különben az azonos prefixű \nes{}eket is kiolvasnánk.

A második típusú keresés azonban jelentősen komplikáltabb, hiszen maga a keresés egy olyan dologra (a típusra) hivatkozik, amelyik csak fordítási időben létezik: a típust a Linda kód tartalmazza, de magát a keresést a PostgreSQL--nek kell végrehajtania.
Ennek a problémának megoldására került bevezetésre az előzőekben már ismertetett \texttt{LV\_TYPE} típus.
...
Ha pedig már léteznek a megfelelő függvények és operátorok, akkor könnyen specifikálható a működésük úgy, hogy a típussal való összehasonlítás esetén az adott műveletet csak az érték típusán és a kapott típuson hajtjuk végre: ez pont egyezik az elvárt keresési feltételünkkel.
...
Az egyik megoldás erre, hogy adott időközönként újra lefuttatjuk a lekérdezést, majd annak eredménye alapján megyünk tovább, vagy ismételten újra keresünk.
...
Emellett még akkor is újra és újra elküldi ugyanazt az eredményt nem adó lekérdezést, ha közben semmi változtatás nem történt a \ts--ben, így biztosan nem fog sikerülni.
...
Összességében a tényleges megoldás a pollolás problémájára, hogy valamilyen várakozást építünk a programba, amelyik addig passzívan várakoztatja a kérdező folyamatot, amíg valamilyen feltétel nem teljesül, ami javítja (vagy garantálja) a lekérdezés sikerességét.
...
Erre a feladatra a tökéletes megoldás a PostgreSQL által nyújtott sajátos funkcionalitás: a \texttt{LISTEN} és \texttt{NOTIFY} SQL parancsok.
